...
LIMIT 4000`;
  ...
  async function runSparql(query) {
    ...
    if (!res.ok) {
      ...
      throw new Error(`SPARQL ${res.status} ${res.statusText}: ${t}`);
    }
    return res.json();
  }

  // --- FIX 1: WKT helper die óók POLYGON/LINESTRING slurpt (eerste coord) ---
  function wktToLatLng(wkt) {
    ...
    m = s.match(/\(\s*([-\d.]+)\s+([-\d.]+)/); // pak eerste coord uit POLYGON/LINESTRING
    if (m) return [parseFloat(m[2]), parseFloat(m[1])];
    return null;
  }

  // Kleine escape helper (vervangt foutieve L.Util.escapeHtml)
  const esc = (v) => {
  ...
  function renderOnMap(rows) {
    ensureMap();
    layer.clearLayers();
    if (!rows.length) { showMsg("Geen resultaten voor deze selectie."); return; }
    ...
    rows.forEach(r => {
      ...
      const html = `
        <div style="min-width:220px">
          <div><strong>${esc(sub)}</strong></div>
          <div>Jaar: ${esc(jaar)}</div>
          ${link ? `<div>RM: <a href="${link}" target="_blank" rel="noopener">${esc(rm)}</a></div>` : ""}
        </div>`;
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    async function run() {
      try {
        ...
        showMsg(`<div class="alert alert-warning">Kon data niet laden: ${esc(e.message)}</div>`);
      } finally {
